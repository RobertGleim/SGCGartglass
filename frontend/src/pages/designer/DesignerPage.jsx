...
const getLinkedParamsFromHash = () => {
  ...
  return {
    template_id: String(params.template || '').trim(),
    pattern_product_id: String(params.pattern_product_id || '').trim(),
    gallery_photo_id: String(params.gallery_photo_id || params.photo_id || '').trim(),
    gallery_template_id: String(params.gallery_template_id || '').trim(),
  };
};
...
const normalizeSectionFills = (input) => {
  if (!input || typeof input !== 'object') return {};
  const out = {};
  Object.entries(input).forEach(([sectionId, sectionValue]) => {
    if (!sectionId || typeof sectionId !== 'string' || !sectionValue || typeof sectionValue !== 'object') return;
    out[sectionId] = {
      ...sectionValue,
      locked: toBool(sectionValue.locked),
    };
  });
  return out;
};
...
const isSectionLocked = (sectionFills, sectionId, sectionNumber) => {
  ...
  if (Number.isFinite(sectionNumber) && sectionNumber > 0) {
    if (toBool(sectionFills[`sec-${sectionNumber}`]?.locked)) return true;
    return Object.values(sectionFills).some((entry) =>
      Number(entry?.sectionNum) === sectionNumber && toBool(entry?.locked)
    );
  }
  return false;
};
...
const getAnchorFromRegionPixels = (pixels, regionId, regionMap, canvasWidth, canvasHeight, preferredX, preferredY) => {
  if (!Array.isArray(pixels) || pixels.length === 0 || !Number.isFinite(canvasWidth) || canvasWidth <= 0) {
    return { x: preferredX, y: preferredY, clearancePx: 0 };
  }
  ...
  for (const pi of pixels) {
    const x = pi % canvasWidth;
    const y = Math.floor(pi / canvasWidth);
    if (x < minX) minX = x;
    if (y < minY) minY = y;
    if (x > maxX) maxX = x;
    if (y > maxY) maxY = y;
  ...
  const hasRegionMap = regionMap instanceof Int32Array && Number.isFinite(canvasHeight) && canvasHeight > 0;
  const isInside = hasRegionMap
    ? (x, y) => {
        const ix = Math.round(x);
        const iy = Math.round(y);
        if (ix < 0 || iy < 0 || ix >= canvasWidth || iy >= canvasHeight) return false;
        return regionMap[(iy * canvasWidth) + ix] === regionId;
      }
    : (x, y) => {
        const ix = Math.round(x);
        const iy = Math.round(y);
  ...
  const interior = getBestInteriorPoint({
    left: minX,
    top: minY,
    right: maxX,
    bottom: maxY,
    fallbackX: preferredX,
    fallbackY: preferredY,
    isInside,
  });

  const sampleStride = Math.max(1, Math.floor(pixels.length / 4000));
  let best = null;
  for (let i = 0; i < pixels.length; i += sampleStride) {
    const pi = pixels[i];
    const x = pi % canvasWidth;
    const y = Math.floor(pi / canvasWidth);
    const dx = x - interior.x;
    const dy = y - interior.y;
    const distSq = (dx * dx) + (dy * dy);
  ...
  if (sampleStride > 1) {
    ...
    const x = last % canvasWidth;
    const y = Math.floor(last / canvasWidth);
    const dx = x - interior.x;
    const dy = y - interior.y;
    const distSq = (dx * dx) + (dy * dy);
    if (!best || distSq < best.distSq) {
  ...
  return best
    ? { x: best.x, y: best.y, clearancePx: interior.clearancePx }
    : { x: interior.x, y: interior.y, clearancePx: interior.clearancePx };
};

const getAnchorForFabricObject = (obj, bounds) => {
  const fallback = {
    x: bounds.left + (bounds.width / 2),
    y: bounds.top + (bounds.height / 2),
  };
  if (!obj || !bounds || bounds.width <= 0 || bounds.height <= 0) return { ...fallback, clearancePx: 0 };
...
export default function DesignerPage() {
        ...
        if (project.template_id) {
          try {
            ...
            if (template) {
              ...
              setStep(STEP.DESIGN);
              if ((!project.design_data || Object.keys(project.design_data).length === 0) && template.default_design_data) {
                woDesignDataRef.current = template.default_design_data;
              }
            }
          } catch (err) {
    ...
    getTemplate(templateId)
      .then((tplRes) => {
        ...
        if (tpl) {
          ...
          if (tpl.default_design_data && typeof tpl.default_design_data === 'object') {
            woDesignDataRef.current = tpl.default_design_data;
          }
          setStep(STEP.DESIGN);
        }
      })
      .catch((err) => {
        console.error('[DesignerPage] Failed to load template defaults mode:', err);
      })
      .finally(() => setLoadingProject(false));
  }, [isAdmin, resetTemplateSession]);
  ...
  useEffect(() => {
    ...
    fetchWO(woId)
      .then(async (res) => {
        ...
        } else if (template?.default_design_data && typeof template.default_design_data === 'object') {
          woDesignDataRef.current = template.default_design_data;
        }
      })
  ...
  useEffect(() => {
    ...
    if (params.project || params.workorder || mode === 'template-defaults') return;
    if (selectedTemplate?.id === templateId && step === STEP.DESIGN) return;
    ...
    getTemplate(templateId)
      .then((templateRes) => {
        ...
        if (!template) return;
        setSelectedTemplate(template);
        if (template.default_design_data && typeof template.default_design_data === 'object') {
          woDesignDataRef.current = template.default_design_data;
        }
        setStep(STEP.DESIGN);
      })
      .catch(() => {
        if (!active) return;
        const fallback = templates.find((entry) => Number(entry?.id) === templateId);
        if (!fallback) return;
        setSelectedTemplate(fallback);
        if (fallback.default_design_data && typeof fallback.default_design_data === 'object') {
          woDesignDataRef.current = fallback.default_design_data;
        }
        setStep(STEP.DESIGN);
      })
      .finally(() => {
      ...
      try {
        ...
        if (templateType === 'image' && selectedTemplate.image_url) {
          ...
          // ── Draw numbered labels on each region ──
          {
            ...
            for (const [regionId, pixels] of regionPixels) {
              ...
              const labelAnchor = getAnchorFromRegionPixels(
                pixels,
                regionId,
                regionMap,
                CANVAS_W,
              ...
              raw.push({
                ...
                clearancePx: labelAnchor.clearancePx,
            ...
            for (let index = 0; index < orderedSections.length; index += 1) {
              ...
              labels.push({
                ...
                w: s.w,
                h: s.h,
                fontPx: getFittedSectionLabelFontPx(sectionNum, s.w, s.h, s.clearancePx),
              });
            }
            setSectionLabels(spreadSectionLabelPositions(labels));
          }
          ...
          const handleClick = (e) => {
            ...
            sectionFillsRef.current[regionId] = {
              ...
              locked: toBool(sectionFillsRef.current?.[regionId]?.locked),
            };
            setFillVersion(v => v + 1);
          };
        ...
        // ── Compute numbered section labels for SVG paths ──
        {
          ...
          canvas.getObjects().forEach((obj, objIndex) => {
            ...
            const anchor = getAnchorForFabricObject(obj, bounds);
            raw.push({
              id: `obj-${objIndex}`,
              obj,
              cx,
        ...
        canvas.on('mouse:down', async (e) => {
          ...
          const gt = activeGlassTypeRef.current;
          const texImg = gt ? textureCacheRef.current.get(gt.id) : null;
          const sectionIdCandidate = hit?._sectionId || hit?.id || hit?.regionId;
          ...
          if (eraseMode) {
            ...
            sectionFillsRef.current[sectionId] = {
              ...
              locked: toBool(sectionFillsRef.current?.[sectionId]?.locked),
            };
            if (sectionNumber) {
              setSelectedLegendNumber(sectionNumber);
            }
  ...
  const handleSaveTemplateDefaults = useCallback(async () => {
    if (!isAdmin || !selectedTemplate?.id) return;
    setSaving(true);
    try {
      ...
      console.error('[DesignerPage] Failed to save template defaults:', err);
      alert('Failed to save template defaults. Please try again.');
    } finally {
      setSaving(false);
    }
  ...
  const activeLinkedGalleryHref = useMemo(() => {
    ...
    params.set('photo_id', activeLinkedResources.gallery_photo_id);
  ...
  const sectionLegendItems = (() => {
    ...
    Object.entries(sectionFillsRef.current || {}).forEach(([sectionId, fill]) => {
      ...
      lockBySection.set(number, toBool(fill?.locked));
    });
  ...
  const toggleSelectedSectionLock = useCallback((targetNumber = null) => {
    const resolvedNumber = Number.isFinite(Number(targetNumber))
      ? Number(targetNumber)
      : selectedLegendNumber;
    if (!isAdmin || resolvedNumber == null) return;
    const sectionEntries = Object.entries(sectionFillsRef.current || {});
    const sectionNumberById = new Map(
      (sectionLabels || []).map((label) => [String(label?.id), Number(label?.num)])
    );
    ...
    for (const [sectionId, fill] of sectionEntries) {
      const storedNum = Number(fill?.sectionNum);
      if (Number.isFinite(storedNum) && storedNum === resolvedNumber) {
        targetSectionId = sectionId;
        break;
      }
      const idMatch = String(sectionId || '').match(/^sec-(\d+)$/);
      if (!targetSectionId && idMatch && Number(idMatch[1]) === resolvedNumber) {
        targetSectionId = sectionId;
      }
      if (!targetSectionId) {
        const mappedNum = Number(sectionNumberById.get(String(sectionId)));
        if (Number.isFinite(mappedNum) && mappedNum === resolvedNumber) {
          targetSectionId = sectionId;
        }
      }
    }
    ...
    if (!targetSectionId) {
      targetSectionId = `sec-${resolvedNumber}`;
    }

    const current = sectionFillsRef.current[targetSectionId] || { sectionNum: resolvedNumber };
    sectionFillsRef.current[targetSectionId] = {
      ...current,
      sectionNum: current.sectionNum || resolvedNumber,
      locked: !toBool(current.locked),
    };
    setSelectedLegendNumber(resolvedNumber);
    setFillVersion(v => v + 1);
  }, [isAdmin, selectedLegendNumber, sectionLabels]);
  ...
  if (step === STEP.GALLERY) {
    return (
      ...
      >
        ...
        {templatesLoading ? (
          <LoadingMessage label="Loading templates" className={styles.loading} />
        ) : (
          <>
            {filtered.length === 0 && (
                  ...
                  >
                    ...
                    <div className={styles.cardInfo}>
                      ...
                      {(() => {
                        ...
                        const linkedTemplateId = String(links.template_id || t.id || '').trim();
                        const linkedGalleryTemplateId = String(links.gallery_template_id || linkedTemplateId).trim();
                        const galleryParams = new URLSearchParams();
                        if (linkedPhotoId) galleryParams.set('photo_id', linkedPhotoId);
                        if (linkedGalleryTemplateId) galleryParams.set('template_id', linkedGalleryTemplateId);
    ...
    >
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.relatedResourceSwitch}>
          ...
          {activeLinkedGalleryHref && (
            <a href={activeLinkedGalleryHref} className={styles.relatedGalleryLink}>
              View photo gallery example{activeLinkedResources.gallery_panel_name ? `: ${activeLinkedResources.gallery_panel_name}` : ''}
            </a>
          )}
        </div>
                  ...
                  {selectedLegendDetails ? (
                    <div className={styles.lastColoredRow}>
                      ...
                      <div>
                        ...
                        <span className={styles.lastGlassLabel}>{selectedLegendDetails.locked ? '🔒 Locked' : '🔓 Unlocked'}</span>
                      </div>
                    </div>
                  ) : (
